...
async function debugDatabase(output: string[]): Promise<void> {
  ...
  try {
    ...
    // Check checkins
    const checkins = await sqlite.execute(
      `SELECT COUNT(*) as count FROM checkins_v1`,
    );
    const checkinCount = checkins.rows && checkins.rows[0]
    ...
    if (logs.rows) {
      logs.rows.forEach((log, i) => {
        output.push(
          `     ${
            i + 1
          }. ${log.run_at}: ${log.events_processed} events, ${log.errors} errors`,
        );
      });
    }
  } catch (error) {
...
async function debugJetstream(output: string[]): Promise<void> {
  ...
  try {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (data.commit?.collection === "app.dropanchor.checkin") {
            checkinEvents++;
            output.push(
              `   🎯 Check-in event: ${data.commit.rkey} (${data.kind}/${data.commit.operation})`,
            );
          }
          ...
          if (data.commit?.collection?.includes("dropanchor")) {
            output.push(
              `   📍 Dropanchor event: ${data.commit.collection} - ${data.commit.rkey}`,
            );
          }
        } catch (e) {
    ...
    if (uniqueCollections.length > 0) {
      output.push(
        `   Collections seen: ${uniqueCollections.slice(0, 5).join(", ")}${
...
async function debugAddressResolution(output: string[]): Promise<void> {
  ...
  try {
    ...
    const testUri =
      "at://did:plc:wxex3wx5k4ctciupsv5m5stb/community.lexicon.location.address/3ltck5h4okl2r";
