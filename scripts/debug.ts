  ...
  try {
    ...
    if (records.length > 0) {
      ...
      if (newFormatCount > 0) {
        ...
        console.log(`   Coordinates: ${newRecord.value.coordinates?.latitude}, ${newRecord.value.coordinates?.longitude}`);
...
async function testApiEndpoints() {
  ...
  const endpoints = [
    ...
    { name: "Nearby (Amsterdam)", url: `${ANCHOR_APPVIEW}/nearby?lat=52.3676&lng=4.9041&radius=5`, expectJson: true }
  ];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (isOk && endpoint.expectJson && contentType.includes('application/json')) {
        const json = await response.json();
        console.log(`   Response: ${JSON.stringify(json).substring(0, 100)}...`);
        
        if (endpoint.name === "Stats") {
          console.log(`   📊 Stats: ${json.totalCheckins} checkins, ${json.totalUsers} users`);
  ...
  results.forEach(result => {
    const status = result.success ? `✅ ${result.status}` : `❌ ${result.error || 'Failed'}`;
