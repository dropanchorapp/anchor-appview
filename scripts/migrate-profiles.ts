...
import {
  batchResolveProfiles,
  ensureProfileCacheTable,
} from "../src/utils/profile-resolver.ts";
...
async function migrateProfiles() {
  ...
  const dids = result.rows
    ? result.rows.map((row) => row.author_did as string)
    : [];
  ...
  for (let i = 0; i < dids.length; i += batchSize) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / batchSize) + 1} of ${
        Math.ceil(dids.length / batchSize)
      }`,
    );
  ...
  console.log(
    `Migration complete! Processed ${processedCount} profiles out of ${dids.length} authors`,
  );
