...
export async function resolveAndCacheAddress(checkinId: string, addressRef: {uri: string, cid?: string}) {
  try {
    // Check cache first
    ...
    if (cached.length > 0) {
      ...
      if (addressData) {
        ...
        console.log('Address resolution failed, marked as failed:', addressRef.uri);
      }
    }
...
async function resolveAddressRecord(uri: string): Promise<any> {
  try {
    ...
    const recordUrl = `${pdsEndpoint}/xrpc/com.atproto.repo.getRecord?repo=${did}&collection=${collection}&rkey=${rkey}`;
    ...
    if (!response.ok) {
      throw new Error(`Record fetch failed: ${response.status} ${response.statusText}`);
    }
...
export async function batchResolveAddresses(addressRefs: Array<{checkinId: string, addressRef: {uri: string, cid?: string}}>) {
  console.log(`Starting batch address resolution for ${addressRefs.length} addresses`);
  ...
  for (const {checkinId, addressRef} of addressRefs) {
    try {
      ...
      console.error(`Failed to resolve address for checkin ${checkinId}:`, error);
      errors++;
    }
  }
  
  console.log(`Batch resolution complete: ${resolved} resolved, ${errors} errors`);
  return { resolved, errors };
}

export async function getUnresolvedAddresses(limit: number = 50): Promise<Array<{checkinId: string, addressRef: {uri: string, cid?: string}}>> {
