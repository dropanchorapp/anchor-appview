...
async function initializeTables() {
  ...
  // Create indexes
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_created ON checkins_v1(created_at DESC)`);
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_author ON checkins_v1(author_did)`);
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_location ON checkins_v1(latitude, longitude)`);
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_follows_follower ON user_follows_v1(follower_did)`);
}

async function getGlobalFeed(url: URL, corsHeaders: CorsHeaders): Promise<Response> {
  ...
  return new Response(JSON.stringify({
    checkins,
    cursor: checkins.length > 0 ? checkins[checkins.length - 1].createdAt : null
  }), { headers: corsHeaders });
}

async function getNearbyCheckins(url: URL, corsHeaders: CorsHeaders): Promise<Response> {
  const lat = parseFloat(url.searchParams.get('lat') || '0');
  const lng = parseFloat(url.searchParams.get('lng') || '0');
  const radius = Math.min(parseFloat(url.searchParams.get('radius') || '5'), 50); // max 50km
  const limit = Math.min(parseInt(url.searchParams.get('limit') || '50'), 100);
  ...
  const results = await sqlite.execute(`
    ...
    LIMIT ?
  `, [limit * 3]); // Get more to filter by distance
  
  // Calculate distances and filter
  const nearbyCheckins = Array.isArray(results) ? results
    .map(row => {
      const distance = calculateDistance(lat, lng, row.latitude as number, row.longitude as number);
      return { ...row, distance };
    })
...
async function getUserCheckins(url: URL, corsHeaders: CorsHeaders): Promise<Response> {
...
async function getFollowingFeed(url: URL, corsHeaders: CorsHeaders): Promise<Response> {
  ...
  const followingDids = Array.isArray(follows) ? follows.map(row => row.following_did) : [];
...
async function getStats(corsHeaders: CorsHeaders): Promise<Response> {
  const [totalCheckins, totalUsers, recentActivity, processingStats] = await Promise.all([
    ...
    sqlite.execute(`SELECT COUNT(*) as count FROM checkins_v1 WHERE created_at > datetime('now', '-24 hours')`),
    sqlite.execute(`SELECT * FROM processing_log_v1 ORDER BY run_at DESC LIMIT 1`)
  ]);
