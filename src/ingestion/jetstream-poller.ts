...
export default async function() {
  ...
  console.log(`Polling completed: ${eventsProcessed} events, ${errors} errors, ${duration}ms`);
...
async function initializeTables() {
  ...
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_created ON ${CHECKINS_TABLE}(created_at DESC)`);
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_author ON ${CHECKINS_TABLE}(author_did)`);
  await sqlite.execute(`CREATE INDEX IF NOT EXISTS idx_checkins_location ON ${CHECKINS_TABLE}(latitude, longitude)`);
}

async function connectAndProcess(): Promise<{eventsProcessed: number, errors: number}> {
...
async function processCheckinEvent(event: any) {
  ...
  const existing = await sqlite.execute(`SELECT id FROM checkins_v1 WHERE id = ?`, [commit.rkey]);
  ...
  if (record.addressRef?.uri) {
    console.log('Address ref found, will resolve later:', record.addressRef.uri);
  }
}