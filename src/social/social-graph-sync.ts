...
export default async function () {
  ...
  try {
    ...
    console.log(
      `Social graph sync completed: ${syncedUsers} users synced, ${errors} errors, ${duration}ms`,
    );
...
async function syncUserFollows(userDid: string) {
  ...
  try {
    ...
    const response = await fetch(
      `https://public.api.bsky.app/xrpc/app.bsky.graph.getFollows?actor=${userDid}&limit=100`,
      {
        headers: {
    ...
    await sqlite.execute(`DELETE FROM user_follows_v1 WHERE follower_did = ?`, [
      userDid,
    ]);
...
async function syncUserFollowsPaginated(userDid: string, cursor: string) {
  try {
    const response = await fetch(
      `https://public.api.bsky.app/xrpc/app.bsky.graph.getFollows?actor=${userDid}&cursor=${cursor}&limit=100`,
      {
        headers: {
          "Accept": "application/json",
          "User-Agent": "Anchor-AppView/1.0",
    ...
    console.log(
      `Synced additional ${insertedCount} follows for user: ${userDid}`,
    );
